...
{                        // order matters
    uint64_t key[4];     // junk
    uint64_t src_addr;   // important - where it reads from - page aligned
    uint64_t dst_addr;   // important - where it writes too - page alignned
    uint64_t num_words;  // write this last - number of 64 byte words
    uint64_t rd_credits; // 8
    uint64_t wr_credits; // 8
};
...
int main(int argc, char *argv[])
{
    ...
    for (uint64_t app = 0; app < num_apps; ++app)
    {
        ...
        aos[app]->aos_cntrlreg_write(0x30, configs[0].num_words); // write this last because it starts the app
    }
